...
class SKConvolution(nn.Module):
    def __init__(self, in_channels, out_channels, M=4, r=16, L=32):
        super(SKConvolution, self).__init__()
        self.M = M
        ...
        self.fc = nn.Sequential(
            nn.Linear(out_channels, max(out_channels // r, L)),
            nn.ReLU(inplace=True),
            nn.Linear(max(out_channels // r, L), out_channels)
        )
    ...
    def forward(self, x):
        ...
        for i, conv in enumerate(self.conv):
            feats.append(conv(x[:,i,:,:,:]))
        U = torch.stack(feats, dim=1)  # (batch_size, M, C, H, W)
        s = self.global_pool(U).squeeze(-1).squeeze(-1)  # (batch_size, M, C)
        z = self.fc(s).view(-1, self.M, self.out_channels)  # (batch_size, M, C)
        z = self.softmax(z) * 0.1
        z = z.unsqueeze(-1).unsqueeze(-1)
        x = (U * z).sum(dim=1) + x[:,-1,:,:,:] * 0.9
        return x
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.mutli_head_atten = nn.MultiheadAttention(hidden_dim, num_heads=2, dropout=ssm_drop_rate)   # (seq_len, batch_size, embed_dim)
        ...
        self.conv33conv33conv11 = nn.Sequential(            # (batch_size, channels, height, width)
        ...
        self.sk_conv = SKConvolution(in_channels=hidden_dim, out_channels=hidden_dim, M=4)  # Adding SK Convolution layer
    ...
    def _forward(self, input: torch.Tensor):
        batch_size, height, width = input.size(0), input.size(1), input.size(2)
        
        # Conv
        input_0 = input.permute(0,3,1,2).contiguous()
        input_0 = self.conv33conv33conv11(input_0)

        # msa
        seq_length = height * width
        input_1 = input.permute(1, 2, 0, 3).contiguous()  
        input_1 = input_1.view(seq_length, input_1.size(-2), input_1.size(-1))
        attention_output, _ = self.mutli_head_atten(input_1, input_1, input_1)
        input_1 = input_1 + attention_output
        input_1 = input_1.view(height, width, input_1.size(-2), input_1.size(-1)).contiguous()
        input_1 = input_1.permute(2,0,1,3).contiguous()

        # mlp
        input_2 = self.mlp(input)
        ...
        input_1 = input_1.permute(0,3,1,2).contiguous()
        input_2 = input_2.permute(0,3,1,2).contiguous()
        x = x.permute(0,3,1,2).contiguous()

        output = torch.cat((input_0.unsqueeze(1),input_1.unsqueeze(1),input_2.unsqueeze(1),x.unsqueeze(1)), dim=1)
        x = self.sk_conv(output)
